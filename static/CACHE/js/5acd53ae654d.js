...
return jQuery;});(function(window){'use strict';function minErr(module,ErrorConstructor){ErrorConstructor=ErrorConstructor||Error;return function(){var SKIP_INDEXES=2;var templateArgs=arguments,code=templateArgs[0],message='['+(module?module+':':'')+code+'] ',template=templateArgs[1],paramPrefix,i;message+=template.replace(/\{\d+\}/g,function(match){var index=+match.slice(1,-1),shiftedIndex=index+SKIP_INDEXES;if(shiftedIndex<templateArgs.length){return toDebugString(templateArgs[shiftedIndex]);}
...
scope[current.resolveAs||'$resolve']=locals;link(scope);}};}})(window,window.angular);(function(window,angular){'use strict';angular.module('ngCookies',['ng']).provider('$cookies',[function $CookiesProvider(){var defaults=this.defaults={};function calcOptions(options){return options?angular.extend({},defaults,options):defaults;}
...
Plugin.call($spy,data)})})}(jQuery);(function(){'use strict';angular.module('user',['user.config','user.routes','user.authentication']);angular.module('user.config',[]);angular.module('user.routes',['$ngRoute']);angular.module('user').run(run);run.$inject=['$http'];function run($http){$http.defaults.xsrfHeaderName='X-CSRFToken';$http.defaults.xsrfCookieName='csrftoken';}})();(function(){'use strict';angular.module('user.config').config(config);config.$inject=['$locationProvider'];function config($locationProvider){$locationProvider.html5Mode(true);$locationProvider.hashPrefix('!');}})();(function(){'use strict';angular.module('user.routes',['ngRoute']).config(config);config.$inject=['$routeProvider'];function config($routeProvider){$routeProvider.when('/',{controller:'RegisterController',controllerAs:'vm',templateUrl:'../static/templates/register.html'}).otherwise({redirectTo:'/'});}})();(function(){'use strict';angular.module('user.authentication',['user.authentication.controllers','user.authentication.services']);angular.module('user.authentication.controllers',[]);angular.module('user.authentication.services',['ngCookies']);})();(function(){'use strict';angular.module('user.authentication.services').factory('Authentication',Authentication);Authentication.$inject=['$cookies','$http'];function Authentication($cookies,$http){var Authentication={register:register};return Authentication;function register(email,password){return $http.post('/accounts/',{email:email,password:password});}}})();(function(){'use strict';angular.module('user.authentication.controllers').controller('RegisterController',RegisterController);RegisterController.$inject=['$location','$scope','Authentication'];function RegisterController($location,$scope,Authentication){var vm=this;vm.register=register;function register(){Authentication.register(vm.email,vm.password);}}})();